...
# File NAME : C:\FLOWORK\tools\email_sender_tools\processor.py total lines 129 
########################################################################
...
class EmailSenderModule(BaseModule, IExecutable, IDataPreviewer):
    ...
    def execute(self, payload: dict, config: dict, status_updater, mode='EXECUTE', **kwargs):
        ...
        if body_template and "{{" in body_template:
            pass
        ...
        if not all([smtp_host, smtp_port, email_address, email_password]):
            return self._error("SMTP credentials missing in Variable Manager (SMTP_HOST, etc).", payload)
        ...
        try:
            ...
            for fpath in paths_to_attach:
                if os.path.exists(fpath) and os.path.isfile(fpath):
                    ...
                    self.logger(f"Attachment not found: {fpath}", "WARN")
    ...
    def get_data_preview(self, config: dict):
        return [{'status': 'ready', 'message': f"To: {config.get('recipient_to', 'Unset')}"}]
